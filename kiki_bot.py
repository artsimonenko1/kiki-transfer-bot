...
# ──────────────────────────────────────────
CARS = {
    "sedan":   {"name": "Седан",     "seats": 2,  "price": 1200},
    "wagon":   {"name": "Универсал", "seats": 4,  "price": 1600},
    "minibus": {"name": "Минибас",   "seats": 10, "price": 1900},
}
...
def order_summary(data, is_bkk=True):
    ...
    flight_txt   = f"\nРейс: {data.get('flight', '—')}" if is_bkk else ""
    board_txt    = (
    ...
    return (
        ...
        f"Автомобиль: {car['name']} — {car['price']} ฿\n"
...
def order_summary_from_row(o):
    ...
    is_bkk   = "Бангкок" in (o['direction'] or '') and (o['direction'] or '').startswith("Бангкок")
    ...
    return (
        f"Направление: {o['direction']}\n"
        f"Автомобиль: {car.get('name', o['car_type'])} — {o['car_price']} ฿\n"
...
def kb_main():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Бангкок — Паттайя", callback_data="dir_bkk")],
...
def kb_cars(prefix="car"):
    ...
    for key, car in CARS.items():
        rows.append([InlineKeyboardButton(
            text=f"{car['name']} — до {car['seats']} мест — {car['price']} ฿",
...
def kb_passengers(max_seats, prefix="pax"):
    ...
    if len(nums) <= 5:
        ...
        rows.append([InlineKeyboardButton(text=str(n), callback_data=f"{prefix}_{n}") for n in nums[5:]])
    rows.append([InlineKeyboardButton(text="Назад", callback_data="back_to_car")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def kb_children():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Назад", callback_data="back_to_pax")],
    ])
...
def kb_bags_large(max_n):
    ...
    rows.append([InlineKeyboardButton(text="Назад", callback_data="back_to_children")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def kb_bags_carry(max_n):
    ...
    if len(btns) > 6:
        rows.append(btns[6:])
    rows = [r for r in rows if r]
    rows.append([InlineKeyboardButton(text="Назад", callback_data="back_to_bags_large")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def kb_confirm():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="Изменить",         callback_data="show_edit_menu"),
        ],
    ])

def kb_edit_menu_bkk():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Рейс",              callback_data="edit_flight")],
        ...
        [InlineKeyboardButton(text="Вернуться к заявке",callback_data="back_to_confirm")],
    ])
...
def kb_admin_order(order_id, direction):
    ...
    rows.append([InlineKeyboardButton(text="Завершить заявку", callback_data=f"adm_done_{order_id}")])
    return InlineKeyboardMarkup(inline_keyboard=rows)

def kb_admin_panel():
    pause_text = "Включить приём заявок" if BOOKING_PAUSED else "Пауза приёма заявок"
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Отзывы клиентов",    callback_data="admin_reviews")],
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "<b>Kiki Transfer приветствует вас.</b>\n\n"
        "Мы организуем комфортный трансфер между аэропортами Бангкока "
        "(Суварнабхуми, Дон Мыанг) и Паттайей.\n\n"
        "Гарантируем встречу с именной табличкой и мониторинг рейса с ожиданием при задержке. "
        "Бронирование осуществляется без предоплаты, все платные дороги уже включены в стоимость. "
        "Оплата производится наличными водителю.\n\n"
        "Выберите действие:",
...
async def dir_bkk(cb: CallbackQuery, state: FSMContext):
    if BOOKING_PAUSED:
        ...
        return
    await state.clear()
    await state.update_data(direction="Бангкок — Паттайя", is_bkk=True)
...
async def handle_car_selection(cb, state, car_key, next_state):
    ...
    await cb.message.edit_text(
        f"<b>{car['name']}</b> — {car['price']} ฿\n\nУкажите количество <b>взрослых пассажиров:</b>",
...
# ──────────────────────────────────────────
#  БКК: РЕЙС — ДАТА — ВРЕМЯ — ОТЕЛЬ — ТЕЛЕФОН — ТАБЛИЧКА — ОПЛАТА
# ──────────────────────────────────────────
...
async def bkk_time(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "<i>Можно добавить адрес или ссылку</i>",
        parse_mode="HTML"
    )
...
async def bkk_board_name(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "Наличными батами — оплата водителю при посадке.\n"
        "Рублями — мы свяжемся с вами для оформления оплаты.",
...
# ──────────────────────────────────────────
#  ПТТ: ДАТА — ВРЕМЯ — АДРЕС — ЗДАНИЕ — ТЕЛЕФОН — ОПЛАТА
# ──────────────────────────────────────────
@dp.message(PTT.date)
...
async def edit_payment_start(cb: CallbackQuery, state: FSMContext):
    data   = await state.get_data()
    is_bkk = data.get("is_bkk", True)
    await cb.message.edit_text("Выберите новый <b>способ оплаты:</b>", reply_markup=kb_payment(), parse_mode="HTML")
...
async def edit_flight_cb(cb: CallbackQuery, state: FSMContext):
    await cb.message.edit_text("Введите новый <b>номер рейса</b>:", parse_mode="HTML")
...
async def send_order(cb: CallbackQuery, state: FSMContext):
    ...
    if payment_method == "rub":
        rub_text = (
            f"Клиент хочет оплатить рублями!\n\n"
            f"Клиент: {user.full_name} ({username})\n"
            f"ID: {user.id}\n"
            f"Заявка #{order_id}\n"
...
async def admin_booked(cb: CallbackQuery):
    ...
    is_bkk   = order['direction'].startswith("Бангкок")
    car      = CARS.get(order['car_type'], {})
    car_info = f"{car.get('name', '')} — {order['car_price']} ฿"
    keys     = order.keys()
    pm       = order['payment_method'] if 'payment_method' in keys else 'cash_thb'
    pay_note = (
        "Оплата наличными водителю при посадке.\n"
        if pm == 'cash_thb'
    ...
    if is_bkk:
        client_text = (
            f"Ваш трансфер #{order_id} забронирован.\n\n"
            f"Направление: {order['direction']}\n"
            f"{car_info}\n"
            f"Дата: {order['travel_date']}, время: {order['travel_time']}\n\n"
            f"Водитель встретит вас с табличкой <b>{order['name_on_board']}</b> "
            f"в зоне прилёта после получения багажа.\n\n"
            f"Мы мониторим ваш рейс и ждём даже при задержке.\n"
            f"{pay_note}"
        ...
        client_text = (
            ...
            f"{car_info}\n"
            f"Дата: {order['travel_date']}, время: {order['travel_time']}\n\n"
            f"Скоро мы пришлём вам фото автомобиля, имя и номер телефона водителя.\n\n"
            f"{pay_note}"
            f"Платные дороги включены.\n\n"
            f"Просьба не курить и не употреблять алкоголь в машине.\n\n"
    ...
    if not is_bkk:
        await cb.message.reply(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                ...
                [InlineKeyboardButton(text="Завершить заявку",          callback_data=f"adm_done_{order_id}")],
            ])
        )
    else:
        await cb.message.reply(
...
async def admin_mark_done(cb: CallbackQuery):
    ...
    try:
        await bot.send_message(
            order['user_id'],
            f"Ваша поездка #{order_id} завершена.\n\n"
            f"Спасибо, что воспользовались Kiki Transfer!\n\n"
            f"Будем рады, если вы оставите отзыв о поездке.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    await cb.message.edit_reply_markup(reply_markup=None)
    await cb.message.reply(
        f"Заявка #{order_id} завершена ({admin_name}). Клиенту отправлен запрос отзыва."
    )
...
async def leave_review_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        f"Оставьте отзыв о поездке #{order_id}.\n\n"
...
async def driver_photo(msg: Message, state: FSMContext):
    ...
    await msg.answer("Введите <b>имя водителя</b>:", parse_mode="HTML")
...
async def driver_phone_handler(msg: Message, state: FSMContext):
    ...
    caption = (
        f"Данные вашего водителя (заявка #{order_id}):\n\n"
        f"Имя: <b>{driver_name_val}</b>\n"
        f"Телефон: <b>{driver_phone_val}</b>\n\n"
        f"{car_info}\n"
        f"Дата: {order['travel_date']}, время: {order['travel_time']}\n"
        f"Адрес подачи: {order['destination']}\n\n"
        f"{pay_note}"
        f"Платные дороги включены.\n\n"
        f"Просьба не курить и не употреблять алкоголь в машине."
    )
...
async def client_message_received(msg: Message, state: FSMContext):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id, admin_text,
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="Ответить клиенту", callback_data=f"adm_msg_{user.id}")]
                ]),
                parse_mode="HTML"
            )
            sent = True
    ...
    if sent:
        await msg.answer(
            "Сообщение отправлено менеджеру. Мы ответим вам в ближайшее время.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_msg_client(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.reply(
        f"Напишите ответ клиенту (ID: <code>{target_id}</code>).\n"
...
async def my_status(msg: Message, state: FSMContext):
    ...
    for o in orders_list:
        ...
        text  += (
            f"<b>Заявка #{o['id']}</b>\n"
            f"Направление: {o['direction']}\n"
            f"Дата: {o['travel_date']} {o['travel_time']}\n"
            f"Автомобиль: {car.get('name','')} — {o['car_price']} ฿\n"
            f"Статус: {status}\n\n"
        )
        if o['status'] == 'done':
            ...
            if not reviews:
                btns.append([InlineKeyboardButton(
                    text=f"Оставить отзыв — заявка #{o['id']}",
...
async def admin_pause(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        "Новые клиенты видят уведомление о паузе. Для возобновления нажмите кнопку ниже.",
        reply_markup=kb_admin_panel(),
        parse_mode="HTML"
    )
...
async def admin_resume(cb: CallbackQuery):
    ...
    await cb.answer("Приём заявок возобновлён", show_alert=True)
...
async def admin_stats(cb: CallbackQuery):
    ...
    for d in s['daily_stats']:
        daily_text += f"  {d['date']}: открытий {d['starts']} | заявок {d['orders']} | выполнено {d['done']}\n"
    ...
    text = (
        ...
        f"  Выполнено: {s['orders_done']}\n"
        f"  Отклонены: {s['orders_rejected']}\n\n"
        "<b>По направлениям:</b>\n"
        f"  Бангкок — Паттайя: {s['bkk_count']}\n"
        f"  Паттайя — Бангкок: {s['ptt_count']}\n\n"
        ...
        f"  Рубли: {s['pay_rub']}\n\n"
        f"<b>Отзывов получено: {s['total_reviews']}</b>\n\n"
        "<b>По дням (7 дней):</b>\n"
        f"{daily_text}"
    )
    await cb.message.edit_text(
...
async def admin_reviews_list(cb: CallbackQuery):
    ...
    for r in reviews:
        ...
        text += (
            f"#{r['id']} | Заявка #{r['order_id']}\n"
            f"Клиент: {r['full_name']} ({username})\n"
            f"{r['text']}\n"
            f"{r['created_at'][:16]}\n\n"
        )
...
async def admin_active(cb: CallbackQuery):
    ...
    for o in orders_list:
        ...
        keys      = o.keys()
        pm        = o['payment_method'] if 'payment_method' in keys else 'cash_thb'
        pay_label_short = "баты" if pm == 'cash_thb' else "рубли"
        text += (
            f"<b>#{o['id']}</b> | {status} | {pay_label_short}\n"
            f"Клиент: {o['full_name']} | Тел.: {o['phone']}\n"
            f"Направление: {o['direction']}\n"
            f"Автомобиль: {car.get('name','')} — {o['car_price']} ฿\n"
            f"Дата: {o['travel_date']} {o['travel_time']}\n"
            f"Адрес: {o['destination']}\n"
            f"Принял: {booked_by}\n\n"
        )
        btns.append([InlineKeyboardButton(
...
async def admin_view_order(cb: CallbackQuery):
    ...
    if order['driver_name']:
        text += f"\n\nВодитель: {order['driver_name']} | Тел.: {order['driver_phone']}"
...
async def admin_done(cb: CallbackQuery):
    ...
    for o in orders_list:
        car          = CARS.get(o['car_type'], {})
        booked_by    = ADMIN_NAMES.get(o['booked_by'], f"ID {o['booked_by']}") if o['booked_by'] else "—"
        status_label = "Выполнена" if o['status'] == 'done' else "Отклонена"
        keys         = o.keys()
        pm           = o['payment_method'] if 'payment_method' in keys else 'cash_thb'
        pay_label_short = "баты" if pm == 'cash_thb' else "рубли"
        text += (
            f"<b>#{o['id']}</b> | {status_label} | {pay_label_short}\n"
            f"Клиент: {o['full_name']}\n"
            f"Направление: {o['direction']}\n"
            f"Автомобиль: {car.get('name','')} — {o['car_price']} ฿\n"
            f"Дата: {o['travel_date']}\n"
            f"Принял: {booked_by}\n\n"
        )
...
#  Любое сообщение вне FSM — пересылается
#  менеджеру, клиенту показывается меню
# ──────────────────────────────────────────
@dp.message()
async def fallback_message(msg: Message, state: FSMContext):
    user        = msg.from_user
    username    = f"@{user.username}" if user.username else "нет username"
    orders_list = get_user_orders(user.id)
    order_id    = orders_list[0]['id'] if orders_list else None
    ...
    save_message(user.id, user.username or "", user.full_name, order_id, msg.text or "[не текст]", 'client_to_admin')
    admin_text = (
        f"Сообщение от клиента (вне меню)\n\n"
        f"Клиент: {user.full_name} ({username})\n"
        f"ID: {user.id}\n"
        + (f"Заявка #{order_id}\n" if order_id else "")
        + f"\n{msg.text or '[не текст]'}"
    )
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
            ...
            logger.error(f"Ошибка пересылки fallback-сообщения: {e}")
...
async def set_bot_commands():
    ...
    await bot.set_my_description(
        "Kiki Transfer приветствует вас.\n\n"
        "Мы организуем комфортный трансфер между аэропортами Бангкока "
        "(Суварнабхуми, Дон Мыанг) и Паттайей.\n\n"
        "Гарантируем встречу с именной табличкой и мониторинг рейса с ожиданием при задержке. "
        "Бронирование осуществляется без предоплаты, все платные дороги уже включены в стоимость. "
    ...
    await bot.set_my_short_description(
        "Трансфер между Бангкоком и Паттайей круглосуточно. Бронирование без предоплаты, встреча в аэропорту с табличкой."
    )
...
async def main():
    init_db()
    logger.info(f"Kiki Transfer Bot v3 запущен. Admins: {ADMIN_IDS}")
